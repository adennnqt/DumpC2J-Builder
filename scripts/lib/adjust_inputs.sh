...
if [ -n "$KERNEL_NAME" ]; then
  ...
  grep -qF "CONFIG_LOCALVERSION=\"$KERNEL_NAME\"" arch/arm64/configs/konoha_defconfig \
    || { echo "[-] sed failed to set CONFIG_LOCALVERSION — pattern not found in konoha_defconfig, check defconfig layout"; exit 1; }
fi
...
if [ "$SPOOF_UNAME" == "on" ]; then
  ...
  grep -qF "CONFIG_KSU_SUSFS_SPOOF_UNAME=y" arch/arm64/configs/konoha_defconfig \
    || { echo "[-] sed failed to enable CONFIG_KSU_SUSFS_SPOOF_UNAME — pattern not found, check defconfig layout"; exit 1; }
  ...
  grep -qF "# CONFIG_KSU_SUSFS_SPOOF_UNAME is not set" arch/arm64/configs/konoha_defconfig \
    || { echo "[-] sed failed to disable CONFIG_KSU_SUSFS_SPOOF_UNAME — pattern not found, check defconfig layout"; exit 1; }
fi
